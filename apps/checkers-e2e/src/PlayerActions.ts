import { PlayerKey, Position } from '@checkers/game-interfaces';
...
export type PlayerActionsType = {
  ...
  setViewedTutorial: (page: Page) => Promise<void>;
...
export class PlayerActions implements PlayerActionsType {
  ...
  async addPlayer() {
    ...
    await this.setViewedTutorial(this.pages[this.pages.length - 1]);
  }

  async setViewedTutorial(page: Page) {
    await page.addInitScript(() => {
      window.localStorage.setItem('checkersTutorial', 'true');
    });
  }
  ...
  async addPlayers() {
    ...
    await this.setViewedTutorial(this.pages[0]);
    await this.setViewedTutorial(this.pages[1]);
  }
